...
function showResults(response, args)
{
    ...
    for(var i = 0; i < response.length; i++)
    {
        ...
        Object.keys(response[i]).forEach(function (element){
            resultsString = resultsString + element + ' ' + response[i][element] + '<br />';
        });
//        {
//            resultsString = resultsString + key + ' ' + response[key] + '<br />';
//        }
        //for(var j = keyArray.length; j < keyArray.length; j++)
//        {
//            //resultsString = resultsString + keyArray[j] + ' ' + response[i][j - (keyArray.length / 2)] + '<br />';
//            resultsString = resultsString + keyArray[j] + ' ' + response[i][j - (keyArray.length)] + '<br />';
//        }
